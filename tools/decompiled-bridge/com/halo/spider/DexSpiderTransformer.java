...
public final class DexSpiderTransformer {
    ...
    public static void main(String[] stringArray) throws Exception {
        ...
        Path path2 = Paths.get(stringArray[1], new String[0]);
        Path path3 = path2.resolveSibling(String.valueOf(path2.getFileName()) + ".raw.jar");
        Files.createDirectories(path2.toAbsolutePath().getParent(), new FileAttribute[0]);
        Files.deleteIfExists(path2);
        Files.deleteIfExists(path3);
        try {
            Dex2jarCmd.main((String[])new String[]{"-f", "-o", path3.toString(), path.toString()});
            DexSpiderTransformer.sanitizeConvertedJar(path3, path2);
            System.err.println("DEBUG: Dex spider transform OK: " + String.valueOf(path) + " -> " + String.valueOf(path2));
        }
        finally {
            Files.deleteIfExists(path3);
        }
    ...
    private static byte[] sanitizeClass(byte[] byArray) {
        ...
        Object object = new SafeClassWriter(3);
