export function humanizeVodError(message: string): string {
  if (/jihulab\.com\/yoursmile2\/TVBox|Failed to download target spider JAR from all candidates/i.test(message)) {
    return "Spider 运行资源获取失败，这批依赖同一个 Spider 的接口会一起无法加载。";
  }
  ...
  if (/dex-only|classes\.dex|Dex spider transform failed|no loadable \.class entries|explicit spider hint not found in JVM classpath/i.test(message)) {
    return "当前 Spider 还停在 dex 兼容阶段，桌面兼容层还没有把它转成可直接执行的形态。";
  }
  return message;
}
