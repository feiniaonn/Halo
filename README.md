...
- 补强了 TVBox 包装页解析链，支持从 HTML 包装页、复制按钮与候选链接中恢复真实源地址，并在源解析失败时提供前端“重新解析”入口。
- 补强了 Spider 兼容运行时与桥接宿主，减少共享 Jar 竞争、初始化误判和桌面端 Android 宿主缺失带来的误报。
- 收紧了音乐会话捕获范围，默认只面向原生音乐平台会话，不再把浏览器里的点播视频误识别成音频播放。
- 收稳了启动、关闭、迷你模式与窗口恢复链，降低“只剩托盘图标”“关闭卡顿”“启动即无响应”等问题的出现概率。
