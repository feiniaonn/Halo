...
pub fn get_ipc_pipe(window_label: &str) -> String {
    ...
    format!("{}_{}_{}_{}", IPC_PIPE_BASE, std::process::id(), window_label, pipe_id)
}
...
pub fn send_command(
    ...
    #[cfg(windows)]
    {
        ...
        let timeout = Duration::from_millis(500); // Wait up to 500ms for the pipe to be ready
        ...
        while start.elapsed() < timeout {
            match OpenOptions::new().read(true).write(true).open(ipc_pipe) {
                ...
                Err(e) => {
                    // 231 is ERROR_PIPE_BUSY, 2 is ERROR_FILE_NOT_FOUND
                    if let Some(os_err) = e.raw_os_error() {
                        if os_err == 231 || os_err == 2 {
        ...
        let pipe = match pipe {
            Some(p) => p,
            None => {
                let err_msg = format!("Timed out waiting for named pipe at '{}' to become available", ipc_pipe);
                error!("For window '{}': {}", window_label, err_msg);
                return Err(crate::Error::IpcError(err_msg));
            }
        };
