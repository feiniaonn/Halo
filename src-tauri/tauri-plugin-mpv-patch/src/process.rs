...
pub fn init_mpv_process<R: Runtime>(
    ...
    let mut instances_lock = match app.mpv().instances.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            log::warn!("mpv instances mutex was poisoned during init, recovering");
            poisoned.into_inner()
        }
    };
    if let Some(instance) = instances_lock.get_mut(window_label) {
        if instance.process.try_wait().unwrap_or(None).is_none() {
            match wait_for_ipc_server(&instance.ipc_pipe, ipc_timeout, window_label) {
                Ok(_) => {
                    info!(
...
pub fn kill_mpv_process<R: Runtime>(app: &AppHandle<R>, window_label: &str) -> crate::Result<()> {
    let instance_to_kill = {
        let mut instances_lock = match app.mpv().instances.lock() {
            Ok(guard) => guard,
            Err(poisoned) => {
                log::warn!("mpv instances mutex was poisoned during destroy, recovering");
                poisoned.into_inner()
            }
        };
